...
All sizes have 300px cells for KenKen-handwritten-v2.
...
def normalize_symbol(base_dir, file_path):
    """Load and normalize operator symbol PNG."""
    symbol_path = base_dir / "symbols" / "operators" / file_path
...
def insert_symbol_in_cage(grid, size, cell, symbol, position, base_dir):
    """Insert operator symbol (computer-generated) into cell."""
    number_size = CELL_SIZE // 4

    symbol_img = Image.fromarray((normalize_symbol(base_dir, symbol + ".png") * 255).astype(np.uint8), mode='L')
...
def write_to_cage(board, size, cage, digit_provider, base_dir):
    ...
    if cage["op"] != "":
        board = insert_symbol_in_cage(board, size, start_cell, cage["op"], len(digits), base_dir)

    return board


def make_board_full(size, puzzle, digit_provider, base_dir):
    """Create complete board with cages and numbers."""
    board = make_board(size)
    for cage in puzzle:
        board = draw_cage(board, size, cage)
        board = write_to_cage(board, size, cage, digit_provider, base_dir)
    return board


def make_and_save(size, puzzle, idx, digit_provider, output_dir, base_dir):
    """Generate and save a board image."""
    board = make_board_full(size, puzzle, digit_provider, base_dir)
...
def main():
    ...
    handwritten_data_dir = base_dir / 'handwritten_data'
    ...
    output_dir = base_dir / 'board_images'
    ...
    for size in sizes:
        ...
        board_size = size * CELL_SIZE
        print(f"\nGenerating {num_puzzles} {size}x{size} puzzles ({board_size}x{board_size}px, {CELL_SIZE}px cells)...")
        ...
        for i in range(num_puzzles):
            ...
            make_and_save(size, cages, i, digit_provider, str(output_dir), base_dir)
            total_generated += 1
