# -*- coding: utf-8 -*-
"""
Advanced data augmentation for handwritten Sudoku/HexaSudoku character recognition.
...
Adapted from KenKen-handwritten-v2 augmentation module.
"""
...
def augment_for_confusion_pairs(img_array, label, level='medium'):
    """
    Extra augmentation for commonly confused digit pairs in Sudoku/HexaSudoku.
    ...
    # HexaSudoku letter confusions (A=10, B=11, etc.)
    elif label in [10, 4]:  # A <-> 4
        if random.random() < 0.3:
            angle = random.uniform(-5, 5)
            img = rotate_image(img, angle)

    elif label in [11, 8]:  # B <-> 8
        if random.random() < 0.3:
            op = random.choice(['erode', 'dilate'])
            img = erode_dilate(img, operation=op, kernel_size=2)
