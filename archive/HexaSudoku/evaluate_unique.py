"""
Evaluate unique HexaSudoku puzzles using the neuro-symbolic pipeline.
...
def main():
    print("=" * 60)
    print("Unique HexaSudoku Neuro-Symbolic Solver Evaluation")
    print("=" * 60)
    ...
    # Load ground truth (unique puzzles)
    with open("./puzzles/unique_puzzles_dict.json", "r") as f:
        puzzles_ds = json.load(f)

    puzzles = puzzles_ds.get('16', [])
    print(f"Loaded {len(puzzles)} unique puzzles from JSON.")

    # Find images
    image_dir = "./unique_board_images"
    ...
    print(f"\nEvaluating {total_puzzles} unique puzzles...")
    print("-" * 60)
    ...
    for idx in range(total_puzzles):
        ...
        # Compare to expected solution (should match exactly for unique puzzles)
    ...
    print(f"  Valid solutions: {total_correct} ({total_correct/total_puzzles*100:.1f}%)")
    print(f"  Exact matches: {total_exact_match} ({total_exact_match/total_puzzles*100:.1f}%)")
    ...
    csv_path = f"{results_dir}/unique_puzzles_detailed_evaluation.csv"
