"""
Train CNN for handwritten Sudoku digit recognition.
...
# Constants
IMG_SIZE = 28
NUM_CLASSES = 10  # 0-9 (0 = empty, 1-9 = digits)
EPOCHS = 30
BATCH_SIZE = 64
LEARNING_RATE = 0.001
...
def save_training_data_csv(images, labels, output_path):
    ...
    with open(output_path, 'w', newline='') as f:
        ...
        for idx in indices:
            ...
            name = "Empty" if label == 0 else str(label)
...
def evaluate_per_class(model, images, labels):
    ...
    for cls in sorted(np.unique(labels)):
        ...
        name = "Empty" if cls == 0 else str(cls)
        print(f"  Class {cls} ({name:5s}): {cls_acc:.4f} ({cls_correct}/{cls_total})")


def main():
    """Main training pipeline."""
    print("=" * 60)
    print("Handwritten Sudoku CNN Training")
    print("=" * 60)
    ...
    csv_path = "./symbols/handwritten_sudoku_data.csv"
    ...
    model_path = f"{model_dir}/handwritten_sudoku_cnn.pth"
