"""
Generate HexaSudoku board images using handwritten TRAINING samples (100-0 Split).

100-0 Split Configuration:
- Board images use TRAINING data (same samples CNN was trained on)
- This tests CNN performance when recognizing digits it has memorized
- Expected result: Near 100% recognition accuracy

This is the KEY DIFFERENCE from 95-5 and 90-10 splits where board images
use held-out TEST data that the CNN never saw during training.
...
# Use same random seed as training for consistency
RANDOM_SEED = 1000
...
def load_training_samples():
    """
    Load TRAINING set handwritten samples (same data used for CNN training).

    This is the key difference from other splits - we use training data
    so the CNN will recognize digits it has already memorized.
    ...
    # Load TRAINING data (not test data)
    # In 100-0 split, these are the same, but we explicitly load train
    train_images = np.load(f"{data_dir}/train_images.npy")
    train_labels = np.load(f"{data_dir}/train_labels.npy")

    print(f"Loaded {len(train_images)} TRAINING samples (100-0 split)")
    print("(These are the SAME samples used for CNN training)")
...
def main():
    ...
    print("100-0 Split (Using TRAINING data for board images)")
    print("=" * 60)
    ...
    if not os.path.exists(puzzles_path):
        raise FileNotFoundError(
            ...
            "Copy from 95-5 split detect handwritten digit errors/puzzles/ first."
        )
    ...
    # Load TRAINING samples (same data CNN was trained on)
    print("\n2. Loading handwritten TRAINING samples...")
    samples_dict = load_training_samples()
    ...
    print("   (Using handwritten TRAINING samples - CNN has memorized these)")
    print("   (This tests upper bound of recognition accuracy)")
    print("-" * 60)
