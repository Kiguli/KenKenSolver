...
- Training: 5700 samples per class
- Test: 300 samples per class
- Random seed: 9505 (different from 90-10 split)

This ensures the test set is completely different from the 90-10 split experiments.
...
# 95/5 split: 5700 train, 300 test per class
MAX_TRAIN_SAMPLES_PER_CLASS = 5700
MAX_TEST_SAMPLES_PER_CLASS = 300
...
def extract_class_samples_shuffled(dataset, source_class, max_samples=None):
    """
    Extract all samples of a specific class from a dataset with shuffling.

    Uses the global random seed to ensure different selection than 90-10 split.
    ...
    # Shuffle with our random seed to get different samples than 90-10
...
def prepare_hexasudoku_data():
    ...
    Uses shuffling with seed 9010 (same as 90-10 split) for direct comparison.
    ...
    print(f"Using random seed {RANDOM_SEED} for shuffling")
    ...
    for digit in range(1, 10):
        ...
        samples = extract_class_samples_shuffled(
            mnist_train,
            source_class=digit,
        ...
        # Test samples (shuffled)
        test_samples = extract_class_samples_shuffled(
            mnist_test,
            source_class=digit,
...
def print_dataset_stats(train_labels, test_labels):
    ...
    print("\n" + "=" * 60)
    print("Dataset Statistics (95-5 Split)")
    print("=" * 60)
...
def main():
    ...
    print("95-5 Train/Test Split")
    print("=" * 60)
    print("\nConfiguration:")
    print(f"  - Random seed: {RANDOM_SEED}")
    print(f"  - Train samples per class: {MAX_TRAIN_SAMPLES_PER_CLASS}")
    print(f"  - Test samples per class: {MAX_TEST_SAMPLES_PER_CLASS}")
    print("\nDatasets:")
